...
def parse_settings_from_gcode(filename):
    ...
    try:
        with open(filename, "r", encoding="utf-8", errors="ignore") as f:
            lines = f.readlines()
        is_preflight = any("generated by preFlight" in l for l in lines[:50])
        ...
        if not (has_exec_start and has_exec_end):
            if not is_preflight:
                ...
                logging.info(
                    "preFlight format detected — no EXECUTABLE_BLOCK markers required"
                )
        ...
        for line in lines:
            ...
            # PreFlight fallback: extract settings from printer_notes,
            # preFlight-specific parameter names, and direct comments
            if is_preflight:
                preflight_param_map = {
                    "temperature": "nozzle_temperature_range_high",
                    "first_layer_temperature": "nozzle_temperature_range_low",
                    "min_print_speed": "slow_down_min_speed",
                    "layer_height": "initial_layer_print_height",
                }
                for line in lines:
                    # Parse mz_flow_temp_* from printer_notes
                    if line.startswith("; ") and "printer_notes" in line and "=" in line:
                        notes_content = line.split("=", 1)[1].strip()
                        for note_line in notes_content.split("\\n"):
                            note_line = note_line.strip()
                            for key in script_settings_keys:
                                if script_settings[key] is not None:
                                    continue
                                if note_line.startswith(key + " ="):
                                    val = note_line.split("=", 1)[1].strip()
                                    if key.endswith("launch_viewer"):
                    ...
                    # Map preFlight parameter names to script parameter names
                    if line.startswith("; ") and " = " in line:
                        content = line[2:].lstrip()
                        for pf_key, script_key in preflight_param_map.items():
                            if print_settings[script_key] is not None:
                                continue
                            if content.startswith(pf_key + " ="):
                                val = content.split("=", 1)[1].strip()
                                try:
                                    print_settings[script_key] = float(val)
                                    logging.info(
                                        f"Found setting in G-code: {script_key} = {print_settings[script_key]}"
                                    )
                                except ValueError:
                                    logging.warning(
                                        f"Could not parse value for {pf_key}: {val}"
                                    )
                    # first_layer_temperature also maps to nozzle_temperature_initial_layer
                    if line.startswith("; ") and "first_layer_temperature" in line and "=" in line:
                        if print_settings.get("nozzle_temperature_initial_layer") is None:
                            val = line.split("=", 1)[1].strip()
                            try:
                                print_settings["nozzle_temperature_initial_layer"] = float(val)
                                logging.info(
                                    f"Found setting in G-code: nozzle_temperature_initial_layer = {print_settings['nozzle_temperature_initial_layer']}"
                                )
                            except ValueError:
                                pass
    except Exception as e:
...
def get_marker_indices(filename):
    ...
    is_preflight = any("generated by preFlight" in l for l in all_lines[:50])
    ...
    # PreFlight without EXECUTABLE_BLOCK markers — use MZ markers directly
    if (exec_start is None or exec_end is None) and is_preflight:
        mz_start = next(
            (i for i, l in enumerate(all_lines)
             if l.strip().startswith("; MZ FLOW TEMP START")), None
        )
        mz_end = next(
            (i for i, l in enumerate(all_lines)
             if l.strip().startswith("; MZ FLOW TEMP END")), None
        )
